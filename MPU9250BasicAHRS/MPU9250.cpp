...
MPU9250::MPU9250()
{
  magbias[0] = -69.;  // User environmental x-axis correction in milliGauss, should be automatically calculated
  magbias[1] = 104.;  // User environmental y-axis correction in milliGauss
  magbias[2] = 47.;  // User environmental z-axis correction in milliGauss
}
...
void MPU9250::getTempData(float * temp)
{
...
{
  float yaw =  atan2(2.0f * (q[1] * q[2] + q[0] * q[3]), q[0] * q[0] + q[1] * q[1] - q[2] * q[2] - q[3] * q[3]) * 180.0f / PI;   
  while (yaw < 0) yaw += 360;
...
{
  return -asin(2.0f * (q[1] * q[3] - q[0] * q[2])) * 180.0f / PI;
}

float MPU9250::getRoll()
{
  return atan2(2.0f * (q[0] * q[1] + q[2] * q[3]), q[0] * q[0] - q[1] * q[1] - q[2] * q[2] + q[3] * q[3]) * 180.0f / PI;
}
...
// measured ones. 
int MPU9250::mahonyQuaternionUpdate()
{
  if (getIntStatus() & 0x01) 
  {  // On interrupt, check if data ready interrupt
    float ax, ay, az, gx, gy, gz, mx, my, mz;

    getAccelData(&ax, &ay, &az);  // Read the x/y/z adc values
    getGyroData(&gx, &gy, &gz);  // Read the x/y/z adc values
    getMagData(&mx, &my, &mz);  // Read the x/y/z adc values
...
void MPU9250::magcalMPU9250(float * dest1, float * dest2) 
{
  ...
  int32_t mag_bias[3] = {0, 0, 0}, 
          mag_scale[3] = {0, 0, 0};
  int16_t mag_max[3] = {0xF000, 0xF000, 0xF000}, 
